...
void xuat(tt x){
	...
	long n;
	    ...
	    printf("|     |                         |                    |                       |                    |                    |\n");
		printf("|%-5s|%-20s|%-20s|%-20s|%-25s|%-0s\n","STT","       [họ và tên]       ","        [CCCD]     ","       [ngày sinh]     ","    [số tài khoản]","     [số dư cuối]   |");
		printf("|_____|_________________________|____________________|_______________________|____________________|____________________|\n");
		long i=0;
...
#define SWAP(type,x,y) do{type tmp = x; x = y; y = tmp;}while(0)
	for(int i=0;i<n-1;i++)
		for(int j=n-1;j>i;j--)
		{
		if (strcmp(a[j].hoten, a[j-1].hoten) > 0) SWAP(tt,a[j],a[j-1]); 
		}
		i=0;
// viết lại vào mảng
	p=fopen("mobile1","wb");
...
#define SWAP(type,x,y) do{type tmp = x; x = y; y = tmp;}while(0)
	for(int i=0;i<n-1;i++)
		for(int j=n-1;j>i;j--)
		{
		if (strcmp(a[j].cancuoccongdan, a[j-1].cancuoccongdan) > 0) SWAP(tt,a[j],a[j-1]); 
		}
		i=0;
// viết lại vào mảng
	p=fopen("mobile1","wb");
...
void xuatkhoataikhoan1()
{
		...
		while(i<n){
			...
			++i;
		printf("|     |                         |                    |                            |                    |\n");
		printf("|%-5d|%-25s|%-20s|%-28s|%-20s|%12.1f |",i,x.hoten,x.cancuoccongdan,x.ngaythangnamsinh,x.stk,x.money);
		printf("|_____|_________________________|____________________|____________________________|____________________|\n");}
		fclose(p);
}
// khóa tài khoản 2
void khoataikhoan2()
...
fflush(stdin);
for(int k=0;k<n;k++){
		if((molsgd=fopen(a[k].stk,"rb"))==NULL&&a[k].money<100&&a[k].money>=0)
		{
			fwrite(&a[k],size,1,f);
			fclose(molsgd);
		for(int m=k;m<n-1;m++){
...
void chuyentien(tt x)
{
	...
	tt a[n];
	fflush(stdin);
	printf("vui lòng nhập vào số tài khoản chuyển đi:");
	gets(c);
	printf("vui lòng nhập số tài khoản chuyển đến:");
	gets(d);
	rewind(p);
	while(i<n)
		{
	...
	fclose(p);
	i=0;
	// kiểm tra xem số tài khoản này có tồn tại trong danh sách không.
	while (i<n)
	{
		if(strcmp(c,a[i].stk)==0)	goto loop3;
		i++;
	}
	printf("không tìm thấy khách hàng này trong danh sách!");
	return;
	loop3:;
	int j=0;
	while (j<n)
	{
		if(strcmp(d,a[j].stk)==0)	goto loop4;
		j++;
	}
	printf("không tìm thấy khách hàng này trong danh sách!");
	return;
	...
	while(true)
	{
			char *sotien;
			system("cls");
			printf("thông tin khách hàng chuyển đi");
			printf("\n");
			printf("họ và tên:%20s\t\tsố tài khoản:%20s\t\tsố tiền trong tài khoản:%.5f",a[i].hoten,a[i].stk,a[i].money);
			printf("\n");
			fflush(stdin);
			printf("thông tin khách hàng chuyển đến");
			printf("\n");
			printf("họ và tên:%20s\t\tsố tài khoản:%20s\t\tsố tiền trong tài khoản:%.5f",a[j].hoten,a[j].stk,a[j].money);
			printf("\n");
			...
			if(k==1)
			{
				if(a[i].money>=(atof(sotien)+100))
					{
						a[i].money=a[i].money-atof(sotien);
						a[j].money=a[j].money+atof(sotien);
					}
				else 
				{
					printf("số tiền không đủ để thực hiện");
					return;
				}	
			printf("đã thực hiện giao dịch thành công đến số tài khoản %s",d);
...
{	
	...
	{	
		system("cls");
		menuif();
		printf("\n Vui lòng bấm nút tương ứng để chọn một trong các mục sau! \n");
		int i;
		scanf("%d",&i);
